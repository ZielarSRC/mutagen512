...
// SHA-256 constants optimized for Sapphire Rapids cache
...
// AVX-512 optimized macros for Sapphire Rapids
#define ROR512(x, n) \
...
// Enhanced round function with better instruction scheduling for Sapphire
// Rapids
...
void Transform(__m512i* state, const uint8_t* data[16]) {
  ...
  // Load message words with optimized memory access patterns for Sapphire
  // Rapids
  for (int t = 0; t < 16; t++) {
    uint32_t wt[16];
    for (int j = 0; j < 16; j++) {
    ...
    W[t] = _mm512_loadu_si512((__m512i*)wt);
  }

  // Unrolled main loop for better instruction pipeline utilization
  for (int t = 0; t < 16; t++) {
    Round512(a, b, c, d, e, f, g, h, _mm512_set1_epi32(K[t]), W[t]);
  }

  // Extended message schedule with vectorized computation
  for (int t = 16; t < 64; t++) {
  ...
  // Update state with AVX-512 arithmetic
...
void sha256avx512_16B(
  ...
  // Extract results with optimized memory stores
  ALIGN64 uint32_t digest[8][16];
  for (int i = 0; i < 8; i++) {
    _mm512_store_si512((__m512i*)digest[i], state[i]);
  }

  // Convert to big-endian and store final hashes
  for (int i = 0; i < 16; i++) {
    for (int j = 0; j < 8; j++) {
