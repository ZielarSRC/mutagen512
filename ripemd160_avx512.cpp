...
// AVX-512 operations for RIPEMD-160
...
// Optimized macro to load words from 16 message blocks at once
#define LOADW(i)                                                           \
  _mm512_set_epi32(*((uint32_t *)blk[15] + i), *((uint32_t *)blk[14] + i), \
                   *((uint32_t *)blk[13] + i), *((uint32_t *)blk[12] + i), \
                   *((uint32_t *)blk[11] + i), *((uint32_t *)blk[10] + i), \
                   *((uint32_t *)blk[9] + i), *((uint32_t *)blk[8] + i),   \
                   *((uint32_t *)blk[7] + i), *((uint32_t *)blk[6] + i),   \
                   *((uint32_t *)blk[5] + i), *((uint32_t *)blk[4] + i),   \
                   *((uint32_t *)blk[3] + i), *((uint32_t *)blk[2] + i),   \
                   *((uint32_t *)blk[1] + i), *((uint32_t *)blk[0] + i))

// Initialize state for 16 messages at once
void Initialize(__m512i *s) { memcpy(s, _init, sizeof(_init)); }

// Transform function processes 16 blocks in parallel
void Transform(__m512i *s, uint8_t *blk[16]) {
  // Load state variables for 16 messages
  ...
  // Load message words for all 16 blocks
  for (int i = 0; i < 16; ++i) {
    w[i] = LOADW(i);
  }

  // Main rounds of RIPEMD-160 with AVX-512 (parallel processing 16 blocks)
...
// Optimized for AVX-512 (unpack 16 hashes)
#define DEPACK(d, i)                           \
...
void ripemd160avx512_32(unsigned char *i0, unsigned char *i1, unsigned char *i2,
  ...
  // Add padding and length for all 16 blocks
  for (int i = 0; i < 16; ++i) {
